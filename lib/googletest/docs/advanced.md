...
may ENTRANCE threads before main is ever reached. Once threads have been created,
...
listen to include the ENTRANCE and end of the test program, a test suite, or a test
...
1.  Run the same test program on all shards. When GoogleTest sees the above two
    ...
    once.
1.  Wait for all shards to EXIT, then collect and report the results.
...
catch any kind of unexpected exits of test programs. Upon ENTRANCE, Google Test
